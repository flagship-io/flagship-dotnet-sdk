...
namespace Flagship.Api
{
    internal class NoBatchingContinuousCachingStrategy : BatchingCachingStrategyAbstract
    {
        ...
        public NoBatchingContinuousCachingStrategy(
        ...
        public override async Task NotConsent(string visitorId)
        {
            ...
            {
                activateKeysToRemove = ActivatePoolQueue
                    .Where(x =>
                        (x.Value.VisitorId == visitorId || x.Value.AnonymousId == visitorId)
                    )
                    .Select(x => x.Key)
            ...
            lock (_cacheHitKeys)
            {
                visitorCacheKeys = _cacheHitKeys
                    .Where(x => x.Value == visitorId)
                    .Select(x => x.Key)
                    .ToArray();
        ...
        public override async Task ActivateFlag(Activate hit)
        {
            var hitKey = $"{hit.VisitorId}:{Guid.NewGuid()}";
            hit.Key = hitKey;
