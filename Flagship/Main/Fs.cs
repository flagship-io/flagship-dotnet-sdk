...
[assembly: InternalsVisibleTo(
    "Flagship.Tests, PublicKey = "
)]
[assembly: InternalsVisibleTo(
...
namespace Flagship.Main
{
    public class Fs
    {
        ...
        private Fs()
        {
            _sdkInitialData = new SdkInitialData() { InstanceId = Guid.NewGuid().ToString() };
        }
        ...
        private IDecisionManager BuildDecisionManager(FlagshipConfig config, HttpClient httpClient)
        {
            ...
            if (config.DecisionMode == DecisionMode.BUCKETING)
            {
                decisionManager = new BucketingManager(
                    (BucketingConfig)config,
                    httpClient,
                    Murmur.MurmurHash.Create32()
                );
        ...
        public static void Start(string envId, string apiKey, FlagshipConfig config = null)
        {
            ...
            if (string.IsNullOrWhiteSpace(envId) || string.IsNullOrWhiteSpace(apiKey))
            {
                ...
                Log.LogError(
                    config,
                    Constants.INITIALIZATION_PARAM_ERROR,
            ...
            var trackingManager = new TrackingManager(
                config,
                httpClient,
                instance._sdkInitialData.InstanceId
            );
            ...
            {
                fsInstance._configManager = new ConfigManager(
                    config,
                    decisionManager,
                    trackingManager
                );
            ...
            instance._sdkInitialData.LastInitializationTimestamp = DateTime
                .Now.ToUniversalTime()
        ...
        public static VisitorBuilder NewVisitor(string visitorId, bool hasConsented)
        {
            ...
            return VisitorBuilder.Builder(
                instance._configManager,
                visitorId,
                hasConsented,
                instance._sdkInitialData
            );
        }
    }
}
